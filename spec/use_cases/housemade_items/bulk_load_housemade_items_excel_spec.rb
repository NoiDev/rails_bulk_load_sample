...
describe 'UseCase: BulkLoadHousemadeItemsExcel' do
    include_context 'a bulk load house-made item scenario'

    let(:use_case) { UseCases::HousemadeItems::BulkLoadHousemadeItemsExcel.new }

    context 'verify spreadsheet only' do
        context 'successful outcome' do
            let(:spreadsheet_file) { File.new('spec/use_cases/housemade_items/spreadsheets/bulk-load-housemade-items--normal.xlsx') }
        ...
        context 'failure outcome' do
            let(:spreadsheet_file) { File.new('spec/use_cases/housemade_items/spreadsheets/bulk-load-housemade-items--missing-fields.xlsx') }
            let(:bulk_load_inventory_items_job) {
                job = create_bulk_load_inventory_items_job(spreadsheet_file.read { |io| io.data }, true)
                job.user = current_user
                job.save!
