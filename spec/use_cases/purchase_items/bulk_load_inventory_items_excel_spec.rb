...
describe 'UseCase: BulkLoadPurchaseItemsExcel' do
    include_context 'a bulk load scenario'

    let(:use_case) { UseCases::PurchaseItems::BulkLoadPurchaseItemsExcel.new }

    context 'verify spreadsheet only' do
        context 'successful outcome' do
            let(:spreadsheet_file) { File.new('spec/use_cases/purchase_items/spreadsheets/bulk-load-inventory-items-no-errors.xlsx') }
        ...
        context 'failure outcome' do
            let(:spreadsheet_file) { File.new('spec/use_cases/purchase_items/spreadsheets/bulk-load-inventory-items-has-errors.xlsx') }
            let(:bulk_load_inventory_items_job) {
                job = create_bulk_load_inventory_items_job(spreadsheet_file.read { |io| io.data }, true)
                job.user = current_user
                job.save!
    ...
    context 'verify and load spreadsheet contents' do
        ...
        context 'successful outcome' do
            ...
            let!(:outcome) { use_case.execute use_case_execution_parameters }
            let(:olives_inventory_item) { PurchasedItem.where(name: 'Olives, bleu cheese stuffed').first }
            let(:cloth_napkins) { 'Cloth napkins' }
            let(:cloth_napkins_inventory_item) { PurchasedItem.where(name: cloth_napkins).first }
            ...
            context 'inventory items' do
                context 'olives, bleu cheese stuffed' do
                    ...
                    it 'is created' do
                        expect(PurchasedItem.where(name: 'Olives, bleu cheese stuffed').count).to eq 1
                    end
                    ...
                    it 'the ordering property is set appropriately' do
                        expect(olives_inventory_item.ordering).to be_truthy
                    end

                    it 'the inventory property is set appropriately' do
                        expect(olives_inventory_item.inventory).to be_truthy
                    end

                    it 'the starting cost is set appropriately' do
                        expect(olives_inventory_item.starting_average_cost).to eq 16.88
                    end
                ...
                context 'cloth napkins' do
                    ...
                    it 'the ordering property is set appropriately' do
                        expect(cloth_napkins_inventory_item.ordering).to be_truthy
                    end

                    it 'the inventory property is set appropriately' do
                        expect(cloth_napkins_inventory_item.inventory).to be_falsey
                    end

                    it 'the starting cost is set appropriately' do
                        expect(cloth_napkins_inventory_item.starting_average_cost).to eq 2.99
                    end
    ...
    context 'multiple vendor/inventory item associations' do
        ...
        let!(:spreadsheet_file) { File.new('spec/use_cases/purchase_items/spreadsheets/bulk-load-inventory-items-multiple-vendors.xlsx') }
        let(:bulk_load_inventory_items_job) {
            job = create_bulk_load_inventory_items_job(spreadsheet_file.read { |io| io.data })
            job.user = current_user
            job.save!
    ...
    context 'duplicate vendor/inventory item associations' do
        ...
        let!(:spreadsheet_file) { File.new('spec/use_cases/purchase_items/spreadsheets/bulk-load-inventory-items-duplicate-items.xlsx') }
        let(:bulk_load_inventory_items_job) {
            job = create_bulk_load_inventory_items_job(spreadsheet_file.read { |io| io.data })
            job.user = current_user
            job.save!
    ...
    context 'duplicate item codes for vendor/inventory item associations' do
        ...
        let!(:spreadsheet_file) { File.new('spec/use_cases/purchase_items/spreadsheets/bulk-load-inventory-items-duplicate-item-codes.xlsx') }
        let(:bulk_load_inventory_items_job) {
            job = create_bulk_load_inventory_items_job(spreadsheet_file.read { |io| io.data })
            job.user = current_user
            job.save!
