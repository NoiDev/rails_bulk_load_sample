...
describe 'UseCase: BulkLoadBeginningCostsCsv' do
    include_context 'a bulk load beginning cost scenario'

    let(:use_case) { UseCases::PurchaseItems::BulkLoadBeginningCostsCsv.new }

    context 'verify spreadsheet only' do
        context 'successful outcome' do
            let(:spreadsheet_file) { File.new('spec/use_cases/purchase_items/spreadsheets/bulk-load-beginning-costs-no-errors.csv') }
        ...
        context 'failure outcome' do
            let(:spreadsheet_file) { File.new('spec/use_cases/purchase_items/spreadsheets/bulk-load-beginning-costs-has-errors.csv') }
            let(:bulk_load_inventory_items_job) {
                job = create_bulk_load_inventory_items_job(spreadsheet_file.read { |io| io.data }, true)
                job.user = current_user
                job.save!
